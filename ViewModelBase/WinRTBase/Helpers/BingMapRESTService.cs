...
{
    public class BingMapRESTApi
    {
        public delegate void OnErrorEventHandler(BingMapRESTApi sender, int statusCode, string statusDescription, string[] errorDetails);
        public event OnErrorEventHandler OnError;

        private string _bingMapKey;
        public BingMapRESTApi(string credentials)
        {
            _bingMapKey = credentials;
        }
        ...
        public async Task<Route> GetRoute(double lat0, double lon0, double lat1, double lon1)
        {
            //Point p0 = new Point { Coordinates = new double[] { lat0, lon0 } };
            //Point p1 = new Point { Coordinates = new double[] { lat1, lon1 } };
            string p0 = string.Format("{0},{1}", lat0.ToString(CultureInfo.InvariantCulture), lon0.ToString(CultureInfo.InvariantCulture));
            string p1 = string.Format("{0},{1}", lat1.ToString(CultureInfo.InvariantCulture), lon1.ToString(CultureInfo.InvariantCulture));
            ...
            {
                if (OnError != null)
                    OnError(this, response.StatusCode, response.StatusDescription, response.errorDetails);
            }
